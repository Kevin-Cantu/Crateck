...
export const ObraCivilSection = () => {
  ...
  return (
    <section id="obra-civil" className="py-24   bg-gradient-to-br from-stone-100 via-slate-100 to-gray-300 dark:from-slate-800 dark:via-slate-900 dark:to-gray-900">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
    {solutions.map(({ icon, title, description }) => (
      <Card
        key={title}
        className="group h-full flex flex-col justify-between bg-white/95 dark:bg-slate-800/95
                   backdrop-blur-sm border-stone-200 dark:border-slate-700 shadow transition-all duration-500
                   hover:shadow-2xl hover:scale-[1.02]"
      >
        <CardHeader className="text-center pb-6">
          <div className="relative mx-auto w-fit">
            <div
              className="bg-gradient-to-br from-stone-200 to-gray-200 dark:from-slate-700 dark:to-gray-700
                         p-5 rounded-2xl transition-all duration-500
                         group-hover:from-stone-300 group-hover:to-gray-300
                         dark:group-hover:from-slate-600 dark:group-hover:to-gray-600"
            >
              <Icon
                name={icon as keyof typeof icons}
                size={36}
                className="text-stone-700 dark:text-stone-300
                           group-hover:text-stone-800 dark:group-hover:text-stone-800
                           transition-colors duration-500"
              />
            </div>
            <div
              className="absolute -inset-2 bg-gradient-to-r from-stone-300 to-gray-300
                         dark:from-slate-600 dark:to-gray-600 rounded-2xl
