...
export const ProyectosCarouselSection = () => {
  ...
  // Responsive: items por página + calcular dots basado en snaps reales de Embla
  const [mounted, setMounted] = useState(false);
  const [itemsPerPage, setItemsPerPage] = useState(2);
  const [dotsCount, setDotsCount] = useState(1);

  const updateLayout = useCallback(() => {
    const per = window.innerWidth < 768 ? 1 : 2; // coincide con flex-basis (100% móvil, 50% desktop)
    setItemsPerPage(per);

    const snaps = emblaApi ? emblaApi.scrollSnapList().length : proyectos.length;
    const pages = Math.max(1, Math.ceil(snaps / per));
    setDotsCount(pages);
  }, [emblaApi]);
  ...
  // Índice de página a resaltar (incluye cola para iluminar último punto)
  const highlightedPageIndex = (() => {
    const per = Math.max(1, itemsPerPage);
    const pages = Math.max(1, dotsCount);
    const pageByIndex = Math.floor((selectedIndex + (per - 1)) / per);
    return Math.min(pages - 1, pageByIndex);
  })();
      ...
      <div className="container -mt-16">
        ...
        <Reveal as="div" delayMs={180} className="relative">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            {/* Indicadores responsive (dots) a la derecha, concuerdan con Cards */}
            {mounted && (
              <div className="flex gap-2">
                {Array.from({ length: dotsCount }).map((_, index) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      const lastStart = Math.max(0, lastSnap - (itemsPerPage - 1));
                      const target = index === dotsCount - 1 ? lastStart : index * itemsPerPage;
                      emblaApi.scrollTo(Math.max(0, Math.min(lastSnap, target)));
                      (document.activeElement as HTMLElement)?.blur();
                    }}
                  ...
                  <Card className="h-full overflow-hidden shadow-md hover:shadow-lg transition-shadow">
                    <CardContent className="p-0 h-full">
                      ...
                      <div className="relative">
                        {/* Grid de imágenes (más anchas al reducir a 2 Cards por fila en desktop) */}
